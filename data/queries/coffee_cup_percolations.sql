select
	...
	pl.player_id,
	pl.player_name
from (select unnest(event_text) as evt, * from data.game_events where array_to_string(event_text, ';') like '%The Saucer Descends%') ge
...
where games.season < 11
	and position(pl.player_name || ' is caught in the Tractor Bean' in ge.evt) > 0
