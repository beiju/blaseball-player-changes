select
	...
	pl.player_id,
	pl.player_name
from (select unnest(event_text) as evt, * from data.game_events where array_to_string(event_text, ';') like '%is Poured Over with%') ge
...
where games.season < 11
	and position(pl.player_name || ' is Poured Over with' in ge.evt) > 0
