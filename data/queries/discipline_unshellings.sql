select
	...
	pl.player_id,
	pl.player_name
from (select unnest(event_text) as evt, * from data.game_events where array_to_string(event_text, ';') like '%they peck %') ge
...
where games.season < 11
	and position('they peck ' || pl.player_name || ' free!' in ge.evt) > 0
