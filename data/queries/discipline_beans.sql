select
	...
	pl.player_id,
	pl.player_name
from (select unnest(event_text) as evt, * from data.game_events where array_to_string(event_text, ';') like '% hits % with chron pitch! % is now %') ge
...
where games.season < 11
	and position('hits ' || pl.player_name || ' with chron pitch!' in ge.evt) > 0

-- The datablase doesn't have Day X, but there was chron bean
union select
    'Jaylen Hotdogfingers hits Wyatt Quitter with chron pitch! Wyatt Quitter is now Repeating!' as evt,
	...
	'3e2882a7-1553-49bd-b271-49cab930d9fc' as game_id, -- technically chron fight id but i'll deal with that later
	8 as season,
	-1 as day, -- uhhhhh
