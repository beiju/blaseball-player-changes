select
	...
	pl.player_id,
	pl.player_name
from (select unnest(event_text) as evt, * from data.game_events where array_to_string(event_text, ';') like '%uses their Free Refill%') ge
...
where games.season < 11
	and position(pl.player_name || ' uses their Free Refill' in ge.evt) > 0
